...
namespace RDIFramework.CodeMaker
{
    ...
    public partial class DBCodeMaker
    {
        ...
        public string BuilderManager(string useDb = null)
        {
            ...
            // 获取主键
            List<ColumnInfo> listColumnInfo = this.GetColumnInfo(this.TableName);
            this.GetPrimaryKey(listColumnInfo);
            this.GetCodeClassManager(listColumnInfo, useDb);
            // this.GetTableColumns(xmlNode);
            this.GetCodeEnd();
            return this.CodeText.ToString();
        }

        private void GetCodeClassManager(List<ColumnInfo> listColumnInfo, string useDb = null)
        {
            bool sortCode = false;
            sortCode = this.ColumnsExists(listColumnInfo, "SortCode");
            ...
            {
                this.CodeText.AppendLine("            if (base.dbProvider == null)");
                this.CodeText.AppendLine("            {");
                this.CodeText.AppendLine("                base.dbProvider = DbFactoryProvider.GetProvider(SystemInfo." + useDb + "Type, SystemInfo." + useDb + "Connection);");
                this.CodeText.AppendLine("            }");
            }
            ...
            // 是否采用了自增量方式？
            bool identity = false;
            if (this.ColumnsExists(listColumnInfo, this.PrimaryKey))
            {
                string pk = GetColumnDataType(listColumnInfo, this.PrimaryKey);
                identity = pk.ToUpper().Equals("INT")
                    || pk.ToUpper().Equals("INTEGER")
                    || pk.ToUpper().Equals("LONG")
                    || GetColumnDataType(listColumnInfo, this.PrimaryKey).Equals("int?");
            }
            if (identity)
            {
            ...
            */
            ...
            createUserId = this.ColumnsExists(listColumnInfo, "CreateUserId");
            bool createBy = false;
            createBy = this.ColumnsExists(listColumnInfo, "CreateBy");
            bool createOn = false;
            createOn = this.ColumnsExists(listColumnInfo, "CreateOn");
            ...
            modifiedUserId = this.ColumnsExists(listColumnInfo, "ModifiedUserId");
            bool modifiedBy = false;
            modifiedBy = this.ColumnsExists(listColumnInfo, "ModifiedBy");
            bool modifiedOn = false;
            modifiedOn = this.ColumnsExists(listColumnInfo, "ModifiedOn");
            ...
            this.GetCodeEntityManager(listColumnInfo);
            this.CodeText.AppendLine("        }");
            this.CodeText.AppendLine(string.Empty);
            this.CodeText.AppendLine("        /// <summary>");
            ...
            this.CodeText.AppendLine(identity
