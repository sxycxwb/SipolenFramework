...
namespace RDIFramework.Utilities
{
    ...
    public class RegexValidatorHelper
    {
        ...
        /// <summary>
        /// 通过给定的正则表达式得到指定分组序号的结果
        /// </summary>
        /// <param name="input">输入</param>
        /// <param name="regExpress">正则表达式</param>
        /// <param name="groupIndex">分组序号</param>
        /// <returns>匹配的结果</returns>
        public static string GetMatchData(string input, string regExpress, int groupIndex)
        {
            string matchData = string.Empty;
            Regex regex = new Regex(regExpress);
            try
            {
                MatchCollection matchCollection = regex.Matches(input);
                if (matchCollection.Count > 0)
                {
                    matchData = matchCollection[0].Groups[groupIndex].Value;
                }
            }
            catch (Exception ex)
            {
