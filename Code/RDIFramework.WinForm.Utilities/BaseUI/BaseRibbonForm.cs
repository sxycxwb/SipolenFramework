...
namespace RDIFramework.WinForm.Utilities
{
    ...
    /// </author> 
    /// </summary>
    public class BaseRibbonForm : Office2007RibbonForm, IBaseForm
    {       
        ...
        public BaseRibbonForm()
        {
            ...
            StyleManager.ChangeStyle((eStyle) Enum.Parse(typeof (eStyle), SystemInfo.CurrentStyle),string.IsNullOrEmpty(SystemInfo.CurrentThemeColor)
                    ? Color.Empty
                    : System.Drawing.ColorTranslator.FromHtml(SystemInfo.CurrentThemeColor));
        }

        public virtual void GetIcon()
        {
            if (System.IO.File.Exists(SystemInfo.AppIco))
            {
                        ...
                        if (SystemInfo.EnableRecordLog)
                        {
                            ...
                            if (SystemInfo.LogOned)
                            {
                                ...
                                if (this.RecordFormLog)
                                {
                                    ...
                                    if (platFormService.LogService is ICommunicationObject)
                                    {
                                        ((ICommunicationObject)platFormService.LogService).Close();
                                    }
                                }
                            }
                        }
        ...
        public bool ModuleIsVisible(string moduleCode)
        {
            ...
            foreach (DataRow dataRow in ClientCache.Instance.DTMoule.Rows)
            {
                ...
                {
                    returnValue = dataRow[PiModuleTable.FieldEnabled].ToString().Equals("1");
                    break;
                }
            }
            //模块是否可见;
        ...
        public bool IsModuleAuthorized(string moduleCode)
        {
            ...
            foreach (DataRow dataRow in ClientCache.Instance.DTMoule.Rows)
            {
                if (dataRow[PiModuleTable.FieldCode].ToString().Equals(moduleCode))
                {
                    returnValue = dataRow[PiModuleTable.FieldIsPublic].ToString().Equals("1");
                    break;
                }
            }
        ...
        {
            string directoryName = SystemInfo.StartupPath + directory;
            ...
            if (!this.DesignMode)
            {
                ...
                if (SystemInfo.EnableRecordLog && SystemInfo.LogOned)
                {
                    ...
                    if (this.RecordFormLog)
                    {
                        ...
                        RDIFrameworkService.LogService.WriteExit(UserInfo, this.LogId);
                        if (RDIFrameworkService.LogService is ICommunicationObject)
                        {
                            ((ICommunicationObject)RDIFrameworkService.LogService).Close();
                        }
                    }
                }
            }
        ...
        {
            DataColumn dataColumn = new DataColumn("colSelected", typeof(System.Boolean));
        ...
        #region public virtual int BindPageData(Control dataControl,UcPageControl pageControl,string tableName, string primaryKey, string orderField, string whereConditional, string queryFields):得到分页数据，同时把绑定到DataGridView控件中。
        /// <summary>
        ...
        /// <returns></returns>
        public virtual int BindPageData(UcDataGridView dataControl, UcPageControl pageControl, string tableName
            ...
            , string queryFields = "*", string connstring = "")
        {
            ...
            {
                pageDataTable = pageData.QueryDataTable(connstring);
            }
            else
            {
                pageDataTable = pageData.QueryDataTable(RDIFramework.Utilities.SystemInfo.BusinessDbConnection);
            }
