var grd,actionWFCommonBizUrl = 'handler/WorkFlowCommonBizHandler.ashx';
...
var mygrid = {
    databind: function (size) {
        grd = $('#list').datagrid({
            url: actionWFCommonBizUrl + '?action=GetMyClaimedTaskList',
            toolbar: '#toolbar',
            width: size.width,
            height: size.height,
            ...
            columns: [[
                ...
                { title: 'WORKFLOWINSID', field: 'WORKFLOWINSID', width: 0, hidden: true },
...
var ToDoTaskMethod = {
    ...
    GiveupClaim: function () {
        ...
        if (row) {
            var query = 'action=GiveupClaimTask&operatorId=' + row.OPERATORINSID + "&workTaskInsId=" + row.WORKTASKINSID;
            $.messager.confirm('询问提示', '确认要放弃认领[' + row.FLOWINSCAPTION + ']吗？', function (data) {
                if (data) {
                    $.ajaxjson(actionWFCommonBizUrl, query, function (d) {
                        if (d.Success) {
                            msg.ok(d.Message);
                            mygrid.reload();
                        } else {
    ...
    HandleTask: function () {
        ...
        if (selectRow) {
            ...
            window.open('ProcessingWorkFlowTask.aspx?' + param, selectRow.FLOWINSCAPTION.replace(/\-/g, ""), 'fullscreen=1,toolbar=no,menubar=no,scrollbars=yes, resizable=yes,location=no, status=no');
        } else {
            ...
            var tmpProcessStepDialog = top.$.hDialog({
                ...
                onOpen: function () {
                    top.$('#tmpGridList').datagrid({
                        url: 'WorkFlow/handler/WorkFlowCommonBizHandler.ashx?action=GetWorkFlowHistory&workFlowInsId=' + workFlowInsId,
                        singleSelect: true,
                        selectOnCheck: true,
                        checkOnSelect: true,
                        width: 700,
