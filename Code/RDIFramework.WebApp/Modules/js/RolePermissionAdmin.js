...
var navgrid,
    ...
    rolePermissionFormUrl           = 'Modules/html/PermissionSet/RolePermissionSet.htm?n=' + Math.random(),
    roleUserBatchSetUrl             = 'Modules/html/PermissionSet/RoleUserBatchSet.htm?n=' + Math.random(),
    permissionScopFormUrl           = 'Modules/html/PermissionSet/PermissionScopForm.htm?n=' + Math.random(),
    roleTableFieldPermissionFormUrl = 'Modules/html/PermissionSet/TableFieldPermission.htm?n=' + Math.random(),
    firstCheckUser                  = '0',
    firstCheckRole                  = '0',
    firstCheckModule                = '0',
...
var RPAdminMethod = {
    ...
    SetRolePermission: function () { //角色权限设置
        ...
        if (curRole) {
            var formDialog = top.$.hDialog({
                title: '角色权限设置', width: 660, height: 600, iconCls: 'icon16_group_key',
                ...
                onLoad: function () {
                    ...
                    top.$('#tableModule').tree({
                        ...
                        onLoadSuccess: function (node, data) {
                            ...
                            var query = 'action=GetModuleByRoleId&roleid=' + curRole.ID;
                        ...
                        onCheck: function (node, checked) {
                            if (firstCheckModule == '0') {
                                if (checked) { //授予当前角色所选模块的访问权限                                           
                                    $.ajaxjson('handler/PermissionHandler.ashx', 'action=SetRoleModulePermission&roleId=' + curRole.ID + '&grantIds=' + node.id, function (d) {
                                        if (d.Data != '1') {
                                            msg.warning('授予角色模块访问权限失败！');
                                        }
                                    });
                                } else { //回收当前角色所选模块的访问权限
                                    $.ajaxjson('handler/PermissionHandler.ashx', 'action=SetRoleModulePermission&roleId=' + curRole.ID + '&revokeIds=' + node.id, function (d) {
                                        if (d.Data != '1') {
                    ...
                    top.$('#tablePermissionItem').tree({
                        ...
                        onLoadSuccess: function (node, data) {
                            ...
                            var query = 'action=GetPermissionItemsByRoleId&roleid=' + curRole.ID;
                        ...
                        onCheck: function (node, checked) {
                            if (firstCheckPermissionItem == '0') {
                                if (checked) { //授予当前角色所选操作权限
                                    $.ajaxjson('handler/PermissionHandler.ashx', 'action=SetRolePermissionItem&roleid=' + curRole.ID + '&grantIds=' + node.id, function (d) {
                                        if (d.Data != '1') {
                                            msg.warning('授予角色操作权限失败！');
                                        }
                                    });
                                } else {  //回收当前角色所选操作权限
                                    $.ajaxjson('handler/PermissionHandler.ashx', 'action=SetRolePermissionItem&roleid=' + curRole.ID + '&revokeIds=' + node.id, function (d) {
                                        if (d.Data != '1') {
            ...
            onLoad: function () {
                ...
                top.$('#tableRole').datagrid({
                    ...
                    onSelect: function (rowIndex, rowData) {
                        if (rowData) {
                            //1、加载当前角色所拥有的用户                            
                            $.ajaxtext('handler/PermissionHandler.ashx', 'action=GetRoleUserIds&roleId=' + rowData.ID, function (data) {
                                firstCheckUser = "1";
                ...
                top.$('#tableUser').datagrid({
                    ...
                    onCheck: function (rowIndex, rowData) { //当前角色的用户赋予
                        ...
                        if (firstCheckUser != '1' && currentRole) {
                            var query = 'action=AddRoleUser&roleId=' + currentRole.ID + '&targetIds=' + rowData.ID;
                    ...
                    onUncheck: function (rowIndex, rowData) { //当前角色的用户收回
                        var currentRole = top.$('#tableRole').datagrid('getSelected');
                        if (firstCheckUser != '1' && currentRole) {
                            var query = 'action=RemoveRoleUser&roleId=' + currentRole.ID + '&targetIds=' + rowData.ID;
                            $.ajaxjson('handler/PermissionHandler.ashx', query, function (d) {
                                if (d.Data != '1') {
                                    msg.warning('回收用户失败！');
                                }
    ...
    SetRoleBatchPermission: function () {  //角色权限批量设置
        var batchPermissionDialog = top.$.hDialog({
            href: 'Modules/html/PermissionSet/RolePermissionBatchSet.htm', width: 1100, height: 630, title: '角色权限批量设置', iconCls: 'icon16_shape_square_key',
            onLoad: function () {
                top.$('#FormContent').layout('panel', 'center').panel({ title: '' });
                //1、绑定界面控件
                //1.1、绑定角色列表与用户列表
                ...
                top.$('#tableRole').datagrid({
                    ...
                    onSelect: function (rowIndex, rowData) {
                        if (rowData) {
                            ...
                            var tmpquery = 'action=GetModuleByRoleId&roleid=' + rowData.ID;
                            $.ajaxtext('handler/PermissionHandler.ashx', tmpquery, function (data) {
                                firstCheckModule = '1';
                                var moduelTree = top.$('#tableModule');
                                moduelTree.tree('uncheckedAll');
                                if (data != '' && data.toString() != '[object XMLDocument]') {
            ...
            var formDialog = top.$.hDialog({
                ...
                onLoad: function () {
                    ...
                    top.$('#tableList').datagrid({
                        url: 'Modules/handler/PermissionHandler.ashx?action=GetAllTableScope&resourceCategory=PIROLE&resourceId=' + currentTableFieldPermissionUser.ID,
                        idField: 'ID',
                        sortName: 'SORTCODE',
                        sortOrder: 'asc',
                        showHeader: false,
                        ...
                        columns: [[
                           ...
                           formatter: function (value, rowData, rowIndex) {
                               return '<img style="cursor:pointer" onclick="permissionTableSet(' + "'PIROLE'" + ',' + "'" + currentTableFieldPermissionUser.ID + "'" + ',' + "'" + rowData.ITEMVALUE + "'" + ',' + value + ')"  src="/Content/css/icon/' + (value ? "checkbox_yes.png" : "checkbox_no.png") + '" />';
                           }
                           },
                    ...
                    top.$('#tableFiledList').datagrid({
                        ...
                        columns: [[
                            ...
                            { title: '字段名称', field: 'COLUMNCODE', width: 140, align: 'left' },
                            {
                                ...
                                formatter: function (value, rowData, rowIndex) {
                                    return '<img style="cursor:pointer" onclick="permissionSet(' + "'" + rowData.ID + "'" + ',' + value + ',' + "'ISPUBLIC'" + ',' + "'PIROLE'" + ',' + "'" + currentTableFieldPermissionUser.ID + "'" + ',' + "'" + top.$('#tableList').datagrid('getSelected').ITEMVALUE + "'" + ',' + "'" + rowData.COLUMNCODE + "'" + ')"  src="/Content/css/icon/' + (value ? "checkbox_yes.png" : "checkbox_no.png") + '" />';
                                }
                            },
                            {
                                field: 'COLUMNACCESS', title: '列访问权限', width: 80, align: 'center',
                                formatter: function (value, rowData, rowIndex) {
                                    return '<img style="cursor:pointer" onclick="permissionSet(' + "'" + rowData.ID + "'" + ',' + value + ',' + "'COLUMNACCESS'" + ',' + "'PIROLE'" + ',' + "'" + currentTableFieldPermissionUser.ID + "'" + ',' + "'" + top.$('#tableList').datagrid('getSelected').ITEMVALUE + "'" + ',' + "'" + rowData.COLUMNCODE + "'" + ')" src="/Content/css/icon/' + (value ? "checkbox_yes.png" : "checkbox_no.png") + '" />';
                                }
                            },
                            {
                                field: 'COLUMNEDIT', title: '列编辑权限', width: 80, align: 'center',
                                formatter: function (value, rowData, rowIndex) {
                                    return '<img style="cursor:pointer" onclick="permissionSet(' + "'" + rowData.ID + "'" + ',' + value + ',' + "'COLUMNEDIT'" + ',' + "'PIROLE'" + ',' + "'" + currentTableFieldPermissionUser.ID + "'" + ',' + "'" + top.$('#tableList').datagrid('getSelected').ITEMVALUE + "'" + ',' + "'" + rowData.COLUMNCODE + "'" + ')" src="/Content/css/icon/' + (value ? "checkbox_yes.png" : "checkbox_no.png") + '" />';
                                }
                            },
                            {
                                field: 'COLUMNDENEY', title: '列拒绝访问', width: 80, align: 'center',
                                formatter: function (value, rowData, rowIndex) {
                                    return '<img style="cursor:pointer" onclick="permissionSet(' + "'" + rowData.ID + "'" + ',' + value + ',' + "'COLUMNDENEY'" + ',' + "'PIROLE'" + ',' + "'" + currentTableFieldPermissionUser.ID + "'" + ',' + "'" + top.$('#tableList').datagrid('getSelected').ITEMVALUE + "'" + ',' + "'" + rowData.COLUMNCODE + "'" + ')" src="/Content/css/icon/' + (value ? "checkbox_yes.png" : "checkbox_no.png") + '" />';
                                }
                            }
                        ]],
            ...
            var tmpSetRoleTableConstraintDialog = top.$.hDialog({
                ...
                onOpen: function () {
                    top.$('#tmpGridList').datagrid({
                        url: 'Modules/handler/PermissionHandler.ashx?action=GetConstraintDT&resourceCategory=PIROLE&resourceId=' + currentRoleTableConstraint.ID,
                        singleSelect: true,
                        selectOnCheck: true,
                        checkOnSelect: true,
                        width: 560,
                                    ...
                                    var tmpSetContraintDialog = top.$.hDialog({//设置约束条件式
                                        ...
                                        submit: function () {
                                            if (top.$('#txtContraint').val()) {
                                                var tmpParam = 'action=SetConstraint&resourceCategory=PIROLE&resourceId=' + currentRoleTableConstraint.ID + '&tableName=' + tmpUserTableConstraintGridRow.TABLECODE + '&tableConstraint=' + top.$('#txtContraint').val();
                            ...
                            handler: function () {
                                ...
                                if (tmpRoleTableConstraintGridRow && tmpRoleTableConstraintGridRow.PERMISSIONCONSTRAINT != null) {
                                    top.$.messager.confirm('询问提示', '确定要删除【' + tmpRoleTableConstraintGridRow.TABLENAME + '】的约束条件吗？', function (data) {
                                        if (data) {
                                            $.ajaxjson('handler/PermissionHandler.ashx', 'action=DeleteConstraint&keyId=' + tmpRoleTableConstraintGridRow.ID, function (d) {
                                                if (d.Data > 0) {
                                                    msg.ok('删除成功！');
                                                    top.$('#tmpGridList').datagrid('reload');
                                                } else {
