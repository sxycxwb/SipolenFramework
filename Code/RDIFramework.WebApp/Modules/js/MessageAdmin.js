...
var mygrid = {
    bindGrid: function (size) {
        navgrid = $('#messageList').datagrid({
            url: 'handler/MessageAdminHandler.ashx?action=GetMessageListByFunctionCode',
...
var MessageAdminMethod = {
    sendMessage: function () {   
        var sendDailog = top.$.hDialog({
            id: 'sendMessage', title: '发送消息', width: 1000, height: 500, href: 'Modules/html/SendMessage.htm', iconCls: 'icon16_comment_edit',
            buttons: [
                    {
                        text: '发 送',
                        ...
                        handler: function () {
                            window.setTimeout(function () {
                                ...
                                $.ajaxjson("handler/MessageAdminHandler.ashx?action=SendMessage", postData, function (d) {
                                    if (d.Success) {
                                        msg.ok(d.Message);
            ...
            onLoad: function () {
                top.$('#Title').focus();
                top.$('#Addresser').html('发件人：' + top.$('#curname').html());
            },
            submit: function () {
                return false;
            }
    ...
    readMessage: function () {
        ...
        if (row) {
            var parm = 'key=' + row.ID;
            $.ajaxjson("handler/MessageAdminHandler.ashx?action=ReadMessage", parm, function (d) {
                if (d.Success) {
            ...
            top.$.messager.alert('Warning', '请选择数据！');
        }
        return false;
    },
    broadcastMessage:function() {
        var broadcastDailog = top.$.hDialog({
            id: 'broadcastMessage', title: '广播消息', width: 1000, height: 470, href: 'Modules/html/BroadcastMessage.htm', iconCls: 'icon16_comments_add',
            buttons: [
                    {
                        text: '发 送',
                        iconCls: 'icon16_email_to_friend',
                                ...
                                top.$.messager.confirm('询问提示', '你确定广播本条消息吗?', function (r) {
                                    if (r) {
                                        window.setTimeout(function () {
                                            ...
                                            $.ajaxjson("handler/MessageAdminHandler.ashx?action=BroadcastMessage", postData, function (d) {
                                                if (d.Success) {
            ...
            onLoad: function () {
                ...
                top.$('#BAddresser').html('发件人：' + top.$('#curname').html());
            },
            submit: function () {
                return false;
            }
    ...
    delMessage: function () {
        ...
        if (row) {
            $.messager.confirm('询问提示', '确认删除选中的数据吗?', function (data) {
                if (data) {
                    var parm = 'key=' + row.ID;
                    $.ajaxjson("handler/MessageAdminHandler.ashx?action=Delete", parm, function (d) {
                        if (d.Data > 0) {
                            msg.ok('删除成功。');
                            mygrid.reload();
                        } else {
