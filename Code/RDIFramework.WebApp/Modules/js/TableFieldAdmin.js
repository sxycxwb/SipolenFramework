        ...
        $('#tbData').datagrid({
            ...
            onClickRow: function (rowIndex, rowData) {
                ...
                $('#tableFieldGird').datagrid({
                    url: actionUrl + '?action=GetDTByTable',
...
var TableFieldAdminMethod = {
    ...
    BatchSet: function () {
        ...
        var ad = top.$.hDialog({
            ...
            toolbar: [
                { text: '全选', iconCls: 'icon-checkbox_yes', handler: function () { TableFieldAdminMethod.btnchecked(true); } },
                { text: '取消全选', iconCls: 'icon-checkbox_no', handler: function () { TableFieldAdminMethod.btnchecked(false); } },
                '-',
                { text: '编辑全部', iconCls: 'icon-pencil', handler: function () { TableFieldAdminMethod.apply('beginEdit'); } },
                { text: '取消编辑', iconCls: 'icon-pencil_delete', handler: function () { TableFieldAdminMethod.apply('cancelEdit'); } },
                '-',
                { text: '应用', iconCls: 'icon-disk_multiple', handler: function () { TableFieldAdminMethod.apply('endEdit'); } }
            ],
            submit: function() {
                ...
                if (data) {
                    ad.dialog('close');
                    $.ajaxtext(actionUrl, 'action=TableFieldBatchSet' + '&data=' + data, function (d) {
                        if (d > 0) {
                            msg.ok('设置成功。');
        ...
        var nb = top.$('#nb').treegrid({
            title: '表字段明细',
            url: 'Modules/handler/TableFieldAdminHandler.ashx?action=GetDTForEditByTable&tableCode=' + curTable.TABLECODE,
    ...
    rowCMenu: function (e, row) { //row 右键菜单
        var createRowMenu = function () {
            ...
            var menus = [
                { title: '编辑并全选', iconCls: '' }, { title: '编辑', iconCls: 'icon16_edit_button' }, '-',
                { title: '全选', iconCls: '' }, { title: '取消全选', iconCls: '' }, '-',
                { title: '取消编辑', iconCls: '' }, { title: '应用', iconCls: 'icon16_save_data' }
            ];
...
function SetTablePermissionMethod() {
    ...
    if (currentRole) {
        var rDialog = top.$.hDialog({
            href: 'Modules/html/SetTablePermission.htm', width: 660, height: 515, title: '设置需要做表权限控件的数据表', iconCls: 'icon16_table_lightning',
            onLoad: function () {
                top.$('#rlayout').layout();
                ...
                top.$('#allDataTables').datagrid({
                    url: 'Modules/handler/TableFieldAdminHandler.ashx?action=GetTableNameAndCodeForPermission',
                ...
                top.$('#selectDataTables').datagrid({
                    url: 'Modules/handler/TableFieldAdminHandler.ashx?action=GetTablePermissionScopeList',
                    onDblClickRow: function (rowIndex, rowData) {
                ...
                top.$('#aSelectTable').click(function () {
                    ...
                    if (_row) {
                        ...
                        if (!hasUserName) {
                            ...
                            //添加权限控制表
                            var query = 'action=AddTablePermissionScope&tableName=' + _row.TABLENAME;
                            $.ajaxtext('handler/TableFieldAdminHandler.ashx', query, function (d) {
                                if (d == "1") {
                ...
                top.$('#aDeleteTable').click(function () {
                    ...
                    if (trow) {
                        ...
                        //移除数据表
                        var query = 'action=DeleteTablePermissionScope&itemvalue=' + trow.ITEMVALUE;
                        $.ajaxtext('handler/TableFieldAdminHandler.ashx', query, function (d) {
                            if (d == "1") {
