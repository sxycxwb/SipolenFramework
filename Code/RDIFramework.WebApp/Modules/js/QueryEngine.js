...
*************************************************************************************
* RDIFramework.NET框架“查询引擎管理”业务界面逻辑
*
* 修改记录：
*   1. 2015-10-20 XuWangBin V3.0 新增本业务逻辑的编写。
*/

var queryEngineGrid,
    controlQueryEngineUrl = 'handler/QueryEngineAdminHandler.ashx',
    formUrl = 'Modules/html/QueryEngine.htm';
...
var queryEngineTree = {
    init: function () {
        $('#queryEngineTree').tree({
            lines: true,
            url: controlQueryEngineUrl + '?action=GetQueryEngineTreeJson&isTree=1',
            animate: true,
            ...
            onSelect: function (node) {
                ...
                $('#queryEngineGrid').treegrid({
                    url: controlQueryEngineUrl + '?action=GetQueryEngineByIds',
...
var crud = {
    ...
    add: function () {
        ...
        var addDialog = top.$.hDialog({
            href: formUrl + '?n=' + Math.random(), title: '添加查询引擎', iconCls: 'icon16_tab_add', width: 430, height: 300,
            onLoad: function () {
                crud.bindCtrl();
                if (treeSelected) {
    ...
    edit: function () {
        ...
        if (row) {
            ...
            var editDailog = top.$.hDialog({
                href: formUrl + '?n=' + Math.random(), title: '修改查询引擎', iconCls: 'icon16_tab_edit', width: 430, height: 300,
                onLoad: function () {
                ...
                submit: function () {
                    if (top.$('#uiform').validate().form()) {
                        ...
                        var queryString = pageMethod.serializeJson(top.$('#uiform'));
                        $.ajaxjson(controlQueryEngineUrl + '?action=SubmitForm&key=' + (row.Id || row.id), queryString, function (d) {
                            if (d.Success) {
                                msg.ok(d.Message);
                                var tmpTree = $('#queryEngineTree');
                                var treeText = top.$('#FullName').val();
