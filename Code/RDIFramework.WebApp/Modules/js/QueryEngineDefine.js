...
*
* 修改记录：
*   1. 2015-10-22 XuWangBin V3.0 新增本业务逻辑的编写。
*/
var queryEngineDefineGrid,
    controlQueryEngineUrl ='handler/QueryEngineDefineAdminHandler.ashx',
    formUrl = 'Modules/html/QueryEngineDefine.htm';
...
var queryEngineTree = {
    init: function () {
        $('#queryEngineTree').tree({
            lines: true,
            url: 'handler/QueryEngineAdminHandler.ashx?action=GetQueryEngineTreeJson&isTree=1',
            animate: true,
...
var crud = {
    ...
    bindCtrl: function (navId) {
        var treeData = '';
        $.ajaxtext('handler/QueryEngineAdminHandler.ashx?action=GetQueryEngineTreeJson', '', function (data) {
            if (data) {
        ...
        top.$('#DataBaseLinkName').combobox({
            url: 'Modules/handler/QueryEngineDefineAdminHandler.ashx?action=GetDataBaseLink',
            method: 'get',
            valueField: 'Id',
            textField: 'LinkName',
            editable: false,
    ...
    add: function () {
        ...
        var addDialog = top.$.hDialog({
            href: formUrl + '?n=' + Math.random(), title: '添加查询引擎', iconCls: 'icon-table_add', width: 660, height: 600,
            onLoad: function () {
                crud.bindCtrl();
                if (treeSelected) {
            ...
            submit: function () {
                if (top.$('#uiform').validate().form()) {
                    ...
                    $.ajaxjson(controlQueryEngineUrl + '?action=SubmitForm', queryString, function (d) {
                        if (d.Success) {
                            msg.ok(d.Message);
    ...
    edit: function () {
        ...
        if (rowEdit) {
            ...
            var editDailog = top.$.hDialog({
                href: formUrl + '?n=' + Math.random(), title: '修改查询引擎', iconCls: 'icon16_table_edit', width: 660, height: 600,
                onLoad: function () {
                    crud.bindCtrl(rowEdit.ID);
                    var parm = 'key=' + rowEdit.ID;
                    $.ajaxjson(controlQueryEngineUrl + '?action=GetEntity', parm, function (data) {
                        if (data) {
                ...
                submit: function () {
                    if (top.$('#uiform').validate().form()) {
                        var queryString = pageMethod.serializeJson(top.$('#uiform'));
                        $.ajaxjson(controlQueryEngineUrl + '?action=SubmitForm&key=' + rowEdit.ID, queryString, function (d) {
                            if (d.Success) {
                                msg.ok(d.Message);
                                grid.reload(queryEngineTree.selected());
    ...
    del: function () {
        ...
        if (row != null) {
            ...
            $.messager.confirm('询问提示', '确认要删除选中的数据吗？', function (data) {
                if (data) {
                    $.ajaxjson(controlQueryEngineUrl + '?action=Delete', query, function (d) {
                        if (d.Success) {
                            msg.ok(d.Message);
                            //重新加载
                            grid.reload(queryEngineTree.selected());
