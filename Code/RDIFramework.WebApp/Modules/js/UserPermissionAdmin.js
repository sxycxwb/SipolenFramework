...
*************************************************************************************
* RDIFramework.NET框架“用户、角色权限设置”综合业务逻辑
*
* 主要完成用户或角色权限的集中批量设置。
* 修改记录：
...
var navgrid,
    ...
    userPermissionFormUrl           = 'Modules/html/PermissionSet/UserPermissionSet.htm?n=' + Math.random(),
    userRoleBatchSetUrl             = 'Modules/html/PermissionSet/UserRoleBatchSet.htm?n=' + Math.random(),
    permissionScopFormUrl           = 'Modules/html/PermissionSet/PermissionScopForm.htm?n=' + Math.random(),
    userTableFieldPermissionFormUrl = 'Modules/html/PermissionSet/TableFieldPermission.htm?n=' + Math.random(),
    firstCheckRole                  = '0',
    firstCheckModule                = '0',
...
var mygrid = {
    bindGrid: function (size) {
        navgrid = $('#list').datagrid({
            url: 'handler/UserAdminHandler.ashx?action=GetUserListByPage',
            toolbar: '#toolbar',
            width: size.width,
            height: size.height,
            idField: 'ID',
...
var UPAdminMethod = {
    ...
    SetUserPermission: function () { //用户权限设置
        ...
        if (curUser) {
            var formDialog = top.$.hDialog({
                title: '用户权限设置', width: 880, height: 600, iconCls: 'icon16_user_level_filtering',
                ...
                onLoad: function () {
                    ...
                    top.$('#tableRole').datagrid({
                        ...
                        onLoadSuccess: function (data) {
                            ...
                            $.ajaxtext('handler/PermissionHandler.ashx', 'action=GetUserRoleIds&userid=' + curUser.ID, function (returndata) {
                            ...
                            top.$('#tableModule').tree({
                                ...
                                onLoadSuccess: function (node, data) {
                                    ...
                                    var query = 'action=GetModuleByUserId&userid=' + curUser.ID;
                                    $.ajaxtext('handler/PermissionHandler.ashx', query, function (returndata) {
                                        if (returndata != '' && returndata.toString() != '[object XMLDocument]') {
                                ...
                                onCheck: function (node, checked) {
                                    if (firstCheckModule == '0') {
                                        if (checked) { //授予当前用户所选模块的访问权限                                           
                                            $.ajaxjson('handler/PermissionHandler.ashx', 'action=SetUserModulePermission&userid=' + curUser.ID + '&grantIds=' + node.id, function (d) {
                                                if (d.Data != '1') {
                                                    msg.warning('授予用户模块访问权限失败！');
                                                }
                                            });
                                        } else { //回收当前用户所选模块的访问权限
                                            $.ajaxjson('handler/PermissionHandler.ashx', 'action=SetUserModulePermission&userid=' + curUser.ID + '&revokeIds=' + node.id, function (d) {
                                                if (d.Data != '1') {
                            ...
                            top.$('#tablePermissionItem').tree({
                                ...
                                onLoadSuccess: function (node, data) {
                                    ...
                                    var query = 'action=GetPermissionItemsByUserId&userid=' + curUser.ID;
                                ...
                                onCheck: function (node, checked) {
                                    if (firstCheckPermissionItem == '0') {
                                        if (checked) { //授予当前用户所选操作权限
                                            $.ajaxjson('handler/PermissionHandler.ashx', 'action=SetUserPermissionItem&userid=' + curUser.ID + '&grantIds=' + node.id, function (d) {
                                                if (d.Data != '1') {
                                                    msg.warning('授予用户操作权限失败！');
                                                }
                                            });
                                        } else {  //回收当前用户所选操作权限
                                            $.ajaxjson('handler/PermissionHandler.ashx', 'action=SetUserPermissionItem&userid=' + curUser.ID + '&revokeIds=' + node.id, function (d) {
                                                if (d.Data != '1') {
                        ...
                        onCheck: function (rowIndex, rowData) { //当前用户角色的赋予
                            if (firstCheckRole != '1') {
                                var query = 'action=AddUserToRole&userid=' + curUser.ID + '&targetIds=' + rowData.ID;
                        ...
                        onUncheck: function (rowIndex, rowData) { //当前用户角色的收回
                            if (firstCheckRole != '1') {
                                var query = 'action=RemoveUserFromRole&userid=' + curUser.ID + '&targetIds=' + rowData.ID;
                                $.ajaxjson('handler/PermissionHandler.ashx', query, function (d) {
                                    if (d.Data != '1') {
                                        msg.warning('回收角色失败！');
                                    }
    ...
    SetUserRole: function () {  //用户角色关联
        ...
        if (currentUser) {
            var rDialog = top.$.hDialog({
                href: 'Modules/html/PermissionSet/UserRoleSet.htm', width: 600, height: 500, title: '用户角色关联', iconCls: 'icon16_group_link',
                onLoad: function () {
                    top.$('#rlayout').layout();
                    ...
                    top.$('#selectedRoles').datagrid({
                        url: 'Modules/handler/RoleAdminHandler.ashx?action=GetRoleListByUserId&userId=' + currentUser.ID,
                        onDblClickRow: function (rowIndex, rowData) {
                    ...
                    top.$('#aSelectRole').click(function () {
                        ...
                        if (_row) {
                            ...
                            if (!hasRealName) {
                                ...
                                //添加角色
                                var query = 'action=AddUserToRole&userId=' + currentUser.ID + '&addRoleIds=' + _row.ID;
                                $.ajaxtext('handler/UserAdminHandler.ashx', query, function (d) {
                                    if (d != "1") {
                                        msg.ok('添加角色失败。');
                                    }
                                });
                    ...
                    top.$('#aDeleteRole').click(function () {
                        ...
                        if (trow) {
                            ...
                            //移除角色
                            var query = 'action=RemoveRoleByUserId&userId=' + currentUser.ID + '&removeRoleIds=' + trow.ID;
                            $.ajaxtext('handler/UserAdminHandler.ashx', query, function (d) {
                                if (d != "1") {
                                    msg.ok('移除角色失败。');
                                }
            ...
            onLoad: function () {
                ...
                navgrid = top.$('#tableUser').datagrid({
                        ...
                        top.$('#tableRole').datagrid({
                            ...
                            onCheck: function (rowIndex, rowData) { //当前用户角色的赋予
                                ...
                                if (firstCheckRole != '1' && currentUser) {
                                    var query = 'action=AddUserToRole&userid=' + currentUser.ID + '&targetIds=' + rowData.ID;
                                    $.ajaxjson('handler/PermissionHandler.ashx', query, function (d) {
                                        if (d.Data != '1') {
                                            msg.warning('授予角色失败！');
                                        }
                            ...
                            onUncheck: function (rowIndex, rowData) { //当前用户角色的收回
                                var currentUser = top.$('#tableUser').datagrid('getSelected');
                                if (firstCheckRole != '1' && currentUser) {
                                    var query = 'action=RemoveUserFromRole&userid=' + currentUser.ID + '&targetIds=' + rowData.ID;
                                    $.ajaxjson('handler/PermissionHandler.ashx', query, function (d) {
                                        if (d.Data != '1') {
                                            msg.warning('回收角色失败！');
                                        }
                    ...
                    onSelect: function (rowIndex, rowData) {
                        if (rowData) {
                            //1、加载当前用户所拥有的角色                            
                            $.ajaxtext('handler/PermissionHandler.ashx', 'action=GetUserRoleIds&userid=' + rowData.ID, function (data) {
                                firstCheckRole = "1";
                                top.$('#tableRole').datagrid('uncheckAll'); //取消所有选中行
    ...
    SetUserBatchPermission: function () {  //用户权限集中批量设置
        var batchPermissionDialog = top.$.hDialog({
            href: 'Modules/html/PermissionSet/UserPermissionBatchSet.htm', width: 1100, height: 630, title: '用户权限批量设置', iconCls: 'icon16_shape_square_key',
            onLoad: function () {
                top.$('#FormContent').layout('panel', 'center').panel({ title: '' });
                //1、绑定界面控件
                //1.1、绑定用户列表与角色列表
                ...
                top.$('#tableUser').datagrid({
                    ...
                    onClickRow: function (rowIndex, rowData) { //选中某一用户后，绑定当前用户所拥有的角色、模块（菜单）访问权限、操作权限
                        ...
                        var curUserModuleIds = [];
                        var query1 = 'action=GetModuleByUserId&userid=' + rowData.ID;
                        $.ajaxtext('handler/PermissionHandler.ashx', query1, function (data) {
                            firstCheckModule = '1';
                            var moduelTree = top.$('#tableModule');
                            moduelTree.tree('uncheckedAll');
                            if (data != '' && data.toString() != '[object XMLDocument]') {
            ...
            var formDialog = top.$.hDialog({
                ...
                onLoad: function () {
                    ...
                    top.$('#tableList').datagrid({
                        url: 'Modules/handler/PermissionHandler.ashx?action=GetAllTableScope&resourceCategory=PIUSER&resourceId=' + currentTableFieldPermissionUser.ID,
                        idField: 'ID',
                        sortName: 'SORTCODE',
                        sortOrder: 'asc',
                        showHeader: false,
                        ...
                        //{ field: 'ck', checkbox: true },
                            ...
                            formatter: function (value, rowData, rowIndex) {
                                return '<img style="cursor:pointer" onclick="permissionTableSet(' + "'PIUSER'" + ',' + "'" + currentTableFieldPermissionUser.ID + "'" + ',' + "'" + rowData.ITEMVALUE + "'" + ',' + value + ')"  src="/Content/css/icon/' + (value ? "checkbox_yes.png" : "checkbox_no.png") + '" />';
                            }
                        },
                        ...
                        onClickRow: function (rowIndex, rowData) {
                            ...
                            top.$('#tableFiledList').datagrid({
                                url: 'Modules/handler/PermissionHandler.ashx?action=GetDTByTable',
                    ...
                    top.$('#tableFiledList').datagrid({
                        ...
                        columns: [[
                            ...
                            { title: '字段名称', field: 'COLUMNCODE', width: 140, align: 'left' },
                            {
                                ...
                                formatter: function (value, rowData, rowIndex) {
                                    return '<img style="cursor:pointer" onclick="permissionSet(' + "'" + rowData.ID + "'" + ',' + value + ',' + "'ISPUBLIC'" + ',' + "'PIUSER'" + ',' + "'" + currentTableFieldPermissionUser.ID + "'" + ',' + "'" + top.$('#tableList').datagrid('getSelected').ITEMVALUE + "'" + ',' + "'" + rowData.COLUMNCODE + "'" + ')" src="/Content/css/icon/' + (value ? "checkbox_yes.png" : "checkbox_no.png") + '" />';
                            ...
                            },
                            {
                                ...
                                formatter: function (value, rowData, rowIndex) {
                                    return '<img style="cursor:pointer" onclick="permissionSet(' + "'" + rowData.ID + "'" + ',' + value + ',' + "'COLUMNACCESS'" + ',' + "'PIUSER'" + ',' + "'" + currentTableFieldPermissionUser.ID + "'" + ',' + "'" + top.$('#tableList').datagrid('getSelected').ITEMVALUE + "'" + ',' + "'" + rowData.COLUMNCODE + "'" + ')" src="/Content/css/icon/' + (value ? "checkbox_yes.png" : "checkbox_no.png") + '" />';
                                }
                            },
                            {
                                field: 'COLUMNEDIT', title: '列编辑权限', width: 80, align: 'center',
                                formatter: function (value, rowData, rowIndex) {
                                    return '<img style="cursor:pointer" onclick="permissionSet(' + "'" + rowData.ID + "'" + ',' + value + ',' + "'COLUMNEDIT'" + ',' + "'PIUSER'" + ',' + "'" + currentTableFieldPermissionUser.ID + "'" + ',' + "'" + top.$('#tableList').datagrid('getSelected').ITEMVALUE + "'" + ',' + "'" + rowData.COLUMNCODE + "'" + ')"  src="/Content/css/icon/' + (value ? "checkbox_yes.png" : "checkbox_no.png") + '" />';
                                }
                            },
                            {
                                field: 'COLUMNDENEY', title: '列拒绝访问', width: 80, align: 'center',
                                formatter: function (value, rowData, rowIndex) {
                                    return '<img style="cursor:pointer" onclick="permissionSet(' + "'" + rowData.ID + "'" + ',' + value + ',' + "'COLUMNDENEY'" + ',' + "'PIUSER'" + ',' + "'" + currentTableFieldPermissionUser.ID + "'" + ',' + "'" + top.$('#tableList').datagrid('getSelected').ITEMVALUE + "'" + ',' + "'" + rowData.COLUMNCODE + "'" + ')" src="/Content/css/icon/' + (value ? "checkbox_yes.png" : "checkbox_no.png") + '" />';
                                }
                            }
                        ]],
                        onLoadSuccess: function (data) {
            ...
            var tmpSetUserTableConstraintDialog = top.$.hDialog({
                ...
                onOpen: function () {
                    top.$('#tmpGridList').datagrid({
                        url: 'Modules/handler/PermissionHandler.ashx?action=GetConstraintDT&resourceCategory=PIUSER&resourceId=' + currentUserTableConstraintUser.ID,
                        singleSelect: true,
                        selectOnCheck: true,
                        checkOnSelect: true,
                        width: 560,
                                    ...
                                    var tmpSetContraintDialog = top.$.hDialog({//设置约束条件式
                                        ...
                                        submit: function () {
                                            if (top.$('#txtContraint').val()) {
                                                var tmpParam = 'action=SetConstraint&resourceCategory=PIUSER&resourceId=' + currentUserTableConstraintUser.ID + '&tableName=' + tmpUserTableConstraintGridRow.TABLECODE + '&tableConstraint=' + top.$('#txtContraint').val();
                                                $.ajaxjson('handler/PermissionHandler.ashx', tmpParam, function (d) {
                                                    if (d.Data > 0) {
                                                        msg.ok('设置成功！');
                            ...
                            handler: function () {
                                ...
                                if (tmpUserTableConstraintGridRow && tmpUserTableConstraintGridRow.PERMISSIONCONSTRAINT != null) {
                                    top.$.messager.confirm('询问提示', '确定要删除【' + tmpUserTableConstraintGridRow.TABLENAME + '】的约束条件吗？', function (data) {
                                        if (data) {
                                            $.ajaxjson('handler/PermissionHandler.ashx', 'action=DeleteConstraint&keyId=' + tmpUserTableConstraintGridRow.ID, function (d) {
                                                if (d.Data > 0) {
                                                    msg.ok('删除成功！');
                                                    top.$('#tmpGridList').datagrid('reload');
                                                } else {
