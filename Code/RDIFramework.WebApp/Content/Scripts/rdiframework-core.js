...
var pageMethod = {
	...
	bindCategory: function(categoryControl, categoryCode) {
		...
		} else if (categoryControl === 'txtCompanyName' ||
			categoryControl === 'txtSubCompanyName' ||
			categoryControl === 'txtDepartmentName' ||
			categoryControl === 'txtSubDepartmentName' ||
			categoryControl === 'txtWorkgroupName') {
			top.$('#' + categoryControl).combobox({
	...
	serializeJson: function(uiFormObj) {
		...
		// return serializeObj;
		return "json=" + JSON.stringify(serializeObj);
	},
		...
		center.panel({
			onResize: function(w, h) {
				if (param.gridType == 'treegrid') {
					$('#' + param.gridId).treegrid('resize', {
						width: w - 6,
						height: h - 36
					});
				} else {
					$('#' + param.gridId).datagrid('resize', {
						width: w - 6,
						height: h - 36
					});
				}
...
function ImageCheckBox(value, options, rowObject) {
    return value ? '<img src="../../Content/css/icon/checkbox_yes.png" alt="⊙" title="是" />' : '<img src="../../Content/css/icon/checkbox_no.png" alt="〇" title="否" />';
};
