...
namespace CRM
{
    ...
    public class CustomerClassService : System.MarshalByRefObject, ICustomerClassService
    {
        private string serviceName = "CustomerClass";
        ...
        public CustomerClassService(CurrentDbType businessDbType, string businessDbConnection):this()
        {
            BusinessDbType       = businessDbType;
            BusinessDbConnection = businessDbConnection;
        }

        #region public string Add(UserInfo userInfo, CustomerClassEntity entity, out string statusCode, out string statusMessage) 新增实体
        /// <summary>
        /// 新增实体
        /// </summary>
        ...
        /// <returns>主鍵</returns>
        public string Add(UserInfo userInfo, CustomerClassEntity entity, out string statusCode, out string statusMessage)
        {
        ...
        public DataTable GetDataTableByPage(UserInfo userInfo, string userId, string searchValue, out int recordCount, int pageIndex = 1, int pageSize = 20, string sortExpression = null, string sortDire = null)
        {
            ...
            using (IDbProvider dbProvider = DbFactoryProvider.GetProvider(BusinessDbType))
            {
                try
                {
                    ...
                    // 取得列表
                    CustomerClassManager manager = new CustomerClassManager(dbProvider, userInfo);
                    dataTable = manager.GetDTByPage(userId, searchValue, out recordCount, pageIndex, pageSize, sortExpression, sortDire);
                    dataTable.TableName = CustomerClassTable.TableName;
                }
                catch (Exception ex)
                {
        ...
        #region public CustomerClassEntity GetEntity(UserInfo userInfo, string id) 取得实体
        /// <summary>
        /// 取得实体
        /// </summary>
        /// <param name="userInfo">用户</param>
        ...
        #region public int Update(UserInfo userInfo, CustomerClassEntity entity, out string statusCode, out string statusMessage) 更新实体
        /// <summary>
        /// 更新实体
        /// </summary>
        /// <param name="userInfo">用户</param>
                ...
                try
                {
                    ...
                    using (IDbProvider dbProvider = DbFactoryProvider.GetProvider(BusinessDbType))
                    {
                        try
                        {
                            ...
                            // 根据条件获取数据
                            CustomerClassManager manager = new CustomerClassManager(dbProvider, userInfo);
                            returnValue = manager.SetProperty(whereParameters, parameters);
                        }
                        catch (Exception ex)
                        {
                            CiExceptionManager.LogException(rdiDbProvider, userInfo, ex);
        ...
        #region public int BatchSave(UserInfo userInfo, List<CustomerClassEntity> entites) 批量存储实体
        /// <summary>
        /// 批量存储实体
        /// </summary>
        /// <param name="userInfo">用户</param>
        ...
        /// <returns>影响行数</returns>
        public int BatchSave(UserInfo userInfo, List<CustomerClassEntity> entites)
        {
            int returnValue = 0;
            ...
            using (IDbProvider rdiDbProvider = DbFactoryProvider.GetProvider(SystemInfo.RDIFrameworkDbType))
            {
                ...
                {
                    rdiDbProvider.Open(RDIFrameworkDbConection);
                    LogManager.Instance.Add(rdiDbProvider, userInfo, this.serviceName, "批量存储实体", MethodBase.GetCurrentMethod());
